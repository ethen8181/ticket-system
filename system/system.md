...
- `TicketSiteCode` The Code 88888 tells us that the ticket was sold by online platform, while all the other site code each represents different physical channel ( e.g. convenience stores, bookstores etc. ).
...
1. [Total Ticket Revenue](#### 1. Total Ticket Revenue)
2. [Mean of SoldPrice by Gender](#### 2. Mean of SoldPrice by Gender)
3. [Age Distribution](#### 3. Age Distribution)
4. [Analysis on the ZipCode column](#### 4. Analysis on the ZipCode column)
5. [Analyze TicketSiteCode](#### 5. Analyze TicketSiteCode)
...
- For this section, we will fist add a new column zipcity to `highdata`. To begin with, we'll extract the first digit of the [zipcode](http://www.easytravel.com.tw/postid_search.asp), the first digit represents the city and the rest are which district in the city, but here we're just going to look at the zipcode from the city-level. 
